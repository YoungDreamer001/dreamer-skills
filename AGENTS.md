...
Re-adapt a local skill after its content in `warehouse/local/<skill-name>/` has been modified:

```bash
./bin/hk-skill adapt <skill-name>
```

When the user says "更新本地技能" or similar phrases (e.g., "重新适配技能", "刷新本地技能"), execute the adapt command for the relevant skill without asking for confirmation. Existing project/global links do not need to be re-enabled because they point to the `warehouse/adapted/` directory, which is regenerated by `adapt`.
